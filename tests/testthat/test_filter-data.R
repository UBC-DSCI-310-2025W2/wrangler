...
# need to assign row labels because the function keeps the original integer row labels
row.names(one_col_output) <- as.integer(c(2, 3, 4)) # https://www.statology.org/change-row-names-in-r/
...
three_cols_output <- data.frame(a = c(2, 3),
                                b = c(3, 4),
                                c = c(3, 4))
row.names(three_cols_output) <- as.integer(c(3, 4))
...
mixed_data_type_output <- data.frame(a = c(2, 3),
                                  numbers = c("two","three"),
                                  numbers2 = c(2, "three"))
row.names(mixed_data_type_output) <- as.integer(c(3, 4))
...
# filter_data(all_zeros) results in a data frame with output:
  # [1] zeros
  # <0 rows> (or 0-length row.names)
# need to set column data type in expected output data frame below as double() to get the same output
all_zeros_output <- data.frame(zeros = double())

all_zeros_character <- data.frame(string = c("0", "0", "0"))
all_zeros_character_output <- data.frame(string = character())
row.names(all_zeros_character_output) <- as.integer()
...
two_row_one_zero_output <- data.frame(a = c(1),
                             b = c(2),
                             c = as.character(c(3)))
row.names(two_row_one_zero_output) <- as.integer(c(2))

no_zero <- data.frame(a = c(1, 2),
                      b = c(3, 4))
no_zero_output <- data.frame(a = c(1, 2),
                             b = c(3, 4))
row.names(no_zero_output) <- as.integer(c(1, 2))
...
no_row_data_frame <- data.frame(a = c())
...
test_that("`filter_data` returns an error when the data frame has no rows", {
  expect_error(filter_data(no_row_data_frame))
})
